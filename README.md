...
- 系统托盘：打开聊天、设置、退出
- 聊天框边缘可拉伸
- 直接说需求即可。核心是：你提需求，灵就动手去做。例如「下午 3 点提醒我吃药」「把 D:\\work\\总结.docx 转成 pdf」「做一个待办网页」。有现成能力会直接用；没有就读写文件、运行命令去试。默认工作目录是项目里的 `LingProjects`。系统目录不会写入。转 PDF 需要 Microsoft Office 或 LibreOffice。聊天框可以点「图」、粘贴或拖入图片；看图会使用 MiniMax-M3。
