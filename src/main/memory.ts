...
export function personaPrompt(memory: UserMemory): string {
  ...
  return [
    ...
    '有现成工具就用现成工具：定时提醒、到点叫人、闹钟一律先走定时任务提醒工具 schedule_reminder（查看用 list_reminders，取消用 cancel_reminder），不要写文件、不要 PowerShell 弹窗、不要 Start-Sleep。工具失败时会返回「定时任务提醒工具调用失败」，再问清时间或换办法。',
    ...
    '工具：schedule_reminder、list_reminders、cancel_reminder、get_workspace、set_workspace、run_command、list_files、read_file、write_file、replace_in_file、find_files、search_in_files、copy_file、create_directory、convert_to_pdf、open_path。相对路径默认相对工作目录。',
